...
- **Meal generation (Phase 7)** stays in-process: a pure domain search over an immutable snapshot. From Sprint 29, search runs in a Web Worker with no Dexie access. Apply always goes through `PlanService` on the main thread. Do not add an optimization-solver or LLM package for this sequence. Record the worker in this file when Sprint 29 ships.
