...
| `UnitRegistry` as a small bundled unit list, not a full unit database                      | Give entry-time units explicit US/metric conventions (`cup-us`, `cup-metric`, `oz-mass`, `oz-fl`) and culinary spoons (`tsp`, `tbsp`). Ambiguous legacy `cup` stays self-only. |
...
- **`fraction.js` + `convert-units`** (Sprints 2 and 4) handle quantity _arithmetic_ only — recipe scaling and compatible-unit aggregation. They do not, and must not be made to, infer package sizes, cooked-vs-dry weight, or interchangeability between different recipes' "servings". Wrap them behind a small `QuantityService` adapter (`scale`/`add`/`format`) so package-specific APIs stay out of the rest of the app; persist plain serializable `Quantity` values (`{ value, unit }`), never library instances.
  - **Known limitation:** `cup-metric` has no convert-units equivalent, so `QuantityService` only aggregates it with another exact `cup-metric`. Legacy unspecified `cup` stays self-only. `tbsp` is a first-class culinary spoon (same as `tsp`) and converts with other known volumes. Do not add alias-map entries to paper over US vs metric cups.
...
- **Localization is presentation.** `uiLocale` and `measurementPreference` (default **as-entered**) must not reinterpret stored quantities, snapshots, or week-start dates. Format numbers and dates with `Intl` in UI; keep arithmetic in domain. UI chrome uses bundled hand-rolled message catalogs (`t` / `tPlural` in `src/ui/localization`); do not add an i18n library. Supported locales are English and Russian. The PWA install name stays English.
- **Grocery aggregation** combines lines only when ingredient identity and a known conversion match. Same culinary unit stays that unit (all tbsp → tbsp). Mixed convertible volumes (tbsp + cup, tsp + tbsp) total in milliliters. Legacy unspecified `cup` does not mix. Generate/update uses live library recipe amounts when the recipe still exists; leftover reuse must not double-count a cooking event.
...
- **Meal generation (Phase 7)** stays in-process: a pure domain search over an immutable snapshot prepared on the main thread. From Sprint 29, `runGeneration` runs in `src/infrastructure/planning/generationWorker.ts` (Vite module worker). The worker may import generation domain functions and `QuantityService`; it must not import Dexie, React, or `ui/`. Apply always goes through `PlanService` on the main thread (`addCookingEventComponents` in one Dexie transaction). Late messages from a canceled or superseded `requestId` are ignored. Do not add an optimization-solver or LLM package for this sequence.
