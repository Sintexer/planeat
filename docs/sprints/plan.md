...
Sprint 15 is done: `Ingredient` gained additive `preferredLabels`/`localizedAliases` fields (locale-scoped, alongside the untouched legacy `aliases` bucket), a `resolveIngredientLabel` display fallback to `name`, and a widened `findCandidatesByName`/`createOrLinkByName` result (`{ ambiguous: true; candidates }`) that surfaces every match instead of silently linking to the first one — collision checks stay scoped to name/aliases/localizedAliases only, since two ingredients may legitimately share a preferred-label word. A new `IngredientNameField` (Fuse-backed, advisory-only autocomplete) replaces the plain ingredient-line `TextInput` in both `RecipeEditor` and `SimpleFoodsScreen`'s create flow, and a new `IngredientCandidateModal` disambiguation step opens whenever a typed name is ambiguous. `IngredientsScreen` gained a `TagsInput` for aliases (replacing the old comma-separated field) and a per-card expandable section for editing preferred labels/locale-scoped aliases. No Dexie migration — additive fields only, backup-schema round-trip verified for locale values outside the current `UI_LOCALES` list.

Sprint 16 is done: `parseIngredientLine` classifies measurements through `UnitRegistry` instead of `QUANTITY_UNITS` plus oz→piece aliases. Cups/tablespoons land as unspecified legacy `cup`/`tbsp`; ounces stay the unrecognized `oz` token until the cook picks `oz-mass`/`oz-fl` or leaves the line unresolved; ranges and non-numeric lines become `quantityText` with `measurementStatus: 'unresolved'` (no low-end invention, no fallback unit `g`). Each imported line keeps `originalText` in the draft and optional `RecipeIngredientLine.sourceText` on save (additive backup field, no Dexie or backup-format bump). `RecipeEditor` shows the original line plus cup/oz confirm controls (Sprint 12 unit selector unchanged); save is allowed with unresolved measurements. Covered by `parseIngredientLine.test.ts` and `normalizeImportedRecipe.test.ts`.

Sprint 17 is done: imported ingredient names no longer auto-create catalog rows on blur. Unique identity matches (`findCandidatesByName`) pre-link without writing aliases; ambiguous names use the candidate modal with an explicit Leave unlinked action; Fuse remains advisory. Linking a phrase that is not already an identifier asks **Add as alias** vs **Link this recipe only** (`IngredientService.addLegacyAlias`, legacy bucket, no locale). `RecipeIngredientLine.ingredientId` is optional so unlinked lines save (`displayText` / `sourceText` kept); grocery generation emits a label-only row. Tests: `IngredientService.test.ts`, `backupSchema.test.ts`, `GroceryService.test.ts`.
