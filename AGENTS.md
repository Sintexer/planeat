...
| `convert-units`                                            | Compatible mass/volume aggregation via `QuantityService.add` / `canConvert` (Sprint 13: resolves units via `UnitRegistry`, excluding `legacy` units from cross-unit resolution — a bare `cup`/`tbsp` only matches itself; `cup-metric` has no library equivalent and stays self-only) |
